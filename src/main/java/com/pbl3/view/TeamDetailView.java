...
public class TeamDetailView extends VerticalLayout implements HasUrlParameter<Long> {
    ...
    public TeamDetailView(TeamMemberService teamMemberService, ProjectTeamService projectTeamService,
                          ProjectService projectService, UserService userService, AuthenticationContext authContext) {
    ...
    private void refreshUI() {
        ...
        this.projectId = currentTeam.getProjectId();
        ...
        boolean isLeader = currentUsername.equals(currentTeam.getLeaderUsername());

        boolean isManager = currentUsername.equals(currentTeam.getManagerUsername());
        ...
        if (currentProjectReadOnly) {
            Span holdNotice = new Span(currentProjectOnHold
                    ? "Dự án đang tạm dừng: chỉ được xem thông tin, không thể thêm/xóa/rời nhóm hoặc chỉnh sửa task."
                    : "Dự án đã hoàn thành: chỉ được xem thông tin, không thể thêm/xóa/rời nhóm hoặc chỉnh sửa task.");
            holdNotice.getStyle().set("color", "var(--lumo-error-color)").set("font-weight", "bold");
            teamInfoSection.add(holdNotice);
        }

        // 3. Phân quyền ẩn hiện nút chức năng chính
        addMemberBtn.setVisible(isManagerOrLeader && !currentProjectReadOnly);
        
        // Nút tự rời nhóm: Chỉ ẩn đối với Leader (Theo logic nghiệp vụ tầng Service)
        leaveTeamBtn.setVisible(!isLeader && !currentProjectReadOnly);
    ...
    private void setupNavigationHeader() {
        ...
        add(new HorizontalLayout(backBtn));
    }
    ...
    private void setupMemberGrid() {
        ...
        memberGrid.addComponentColumn(member -> {
            ...
            removeBtn.setVisible(isManagerOrLeader && !currentProjectReadOnly && !"LEADER".equalsIgnoreCase(member.getRole()));
