...
public class TaskManagementView extends VerticalLayout implements HasUrlParameter<Long> {
    ...
    public void setParameter(BeforeEvent event, Long parameter) {
        ...
        createTaskBtn.setVisible(isManagerOrLeader && !currentProjectReadOnly);
        createTaskBtn.setEnabled(!currentProjectReadOnly);
        createTaskBtn.setTooltipText(currentProjectReadOnly
                ? (currentProjectOnHold ? "Dự án đang tạm dừng, không thể tạo công việc" : "Dự án đã hoàn thành, không thể tạo công việc")
                : "Thêm công việc");
        ...
        taskGrid.addComponentColumn(task -> {
            ...
            if (isAssignee) {
                ...
                if (task.getStatus() == Task.TaskStatus.IN_PROGRESS) {
                    ...
                    Button reqExtensionBtn = new Button("Gia hạn", e -> openExtensionDialog(task));
                    reqExtensionBtn.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL);

                    Button reqChangeBtn = new Button("Đổi việc", e -> openChangeTaskDialog(task));
            ...
            if (isManagerOrLeader) {
                ...
                if (task.getStatus() == Task.TaskStatus.CHANGE_REQUESTED || 
                    ...
                    Button viewReasonBtn = new Button(VaadinIcon.EYE.create(), e -> openViewReasonPopup(task));
                    viewReasonBtn.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL);
                    viewReasonBtn.setTooltipText("Xem nội dung yêu cầu");

                    Button approveRequestBtn = new Button(VaadinIcon.CHECK.create(), e -> {
                        try {
                            taskService.resolveMemberRequest(task.getId(), true);
                            Notification.show("Đã chấp nhận yêu cầu").addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                            refreshGridData();
                        } catch (Exception ex) {
                            Notification.show(ex.getMessage()).addThemeVariants(NotificationVariant.LUMO_ERROR);
                        }
                    });
                    approveRequestBtn.addThemeVariants(ButtonVariant.LUMO_SUCCESS, ButtonVariant.LUMO_ICON, ButtonVariant.LUMO_SMALL);
                    approveRequestBtn.setTooltipText("Chấp nhận yêu cầu");

                    Button rejectRequestBtn = new Button(VaadinIcon.CLOSE.create(), e -> {
                        try {
                            taskService.resolveMemberRequest(task.getId(), false);
                            Notification.show("Đã từ chối yêu cầu").addThemeVariants(NotificationVariant.LUMO_WARNING);
                            refreshGridData();
                        } catch (Exception ex) {
                            Notification.show(ex.getMessage()).addThemeVariants(NotificationVariant.LUMO_ERROR);
                        }
                    });
                    rejectRequestBtn.addThemeVariants(ButtonVariant.LUMO_ERROR, ButtonVariant.LUMO_ICON, ButtonVariant.LUMO_SMALL);
                    rejectRequestBtn.setTooltipText("Từ chối yêu cầu");

                    actions.add(viewReasonBtn, approveRequestBtn, rejectRequestBtn);
                }
    ...
    private void openTaskDialog(ShowTaskResponse taskResponse) {
        if (currentProjectReadOnly) {
            Notification.show(currentProjectOnHold
                    ? "Dự án đang tạm dừng, không thể tạo hoặc chỉnh sửa công việc"
                    : "Dự án đã hoàn thành, không thể tạo hoặc chỉnh sửa công việc")
    ...
    private void openExtensionDialog(ShowTaskResponse task) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Yêu cầu gia hạn công việc");
        dialog.setWidth("480px");

        TextArea reasonArea = new TextArea("Lý do gia hạn");
        reasonArea.setWidthFull();
        reasonArea.setRequired(true);

        DateTimePicker requestedDeadlinePicker = new DateTimePicker("Deadline mong muốn");
        requestedDeadlinePicker.setWidthFull();

        // Deadline mong muốn không được vượt quá deadline nhóm.
        TeamResponse currentTeam = projectTeamService.getAllTeams().stream()
                .filter(t -> t.getTeamId().equals(teamId))
                .findFirst()
                .orElse(null);
        if (currentTeam != null && currentTeam.getDeadline() != null) {
            requestedDeadlinePicker.setMax(currentTeam.getDeadline());
        }
        if (task.getDeadLine() != null) {
            requestedDeadlinePicker.setMin(task.getDeadLine());
        }
        ...
        Button submitBtn = new Button("Gửi yêu cầu", e -> {
            ...
            try {
                taskService.requestExtension(task.getId(), reasonArea.getValue(), requestedDeadlinePicker.getValue());
                Notification.show("Đã gửi yêu cầu gia hạn")
    ...
    private void openChangeTaskDialog(ShowTaskResponse task) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Yêu cầu đổi công việc");
        dialog.setWidth("480px");

        TextArea reasonArea = new TextArea("Lý do đổi công việc");
        reasonArea.setWidthFull();
        reasonArea.setRequired(true);

        ComboBox<TeamMemberResponse> requestedAssigneeCombo = new ComboBox<>("Người thực hiện mới mong muốn");
        List<TeamMemberResponse> members = teamMemberService.getMembersByTeam(teamId);
        requestedAssigneeCombo.setItems(members);
        requestedAssigneeCombo.setItemLabelGenerator(TeamMemberResponse::getFullName);
        requestedAssigneeCombo.setWidthFull();
        members.stream()
                .filter(m -> m.getUsername() != null && !m.getUsername().equals(task.getAssigneeUsername()))
                .findFirst()
                .ifPresent(requestedAssigneeCombo::setValue);

        Button submitBtn = new Button("Gửi yêu cầu", e -> {
            if (reasonArea.isEmpty() || requestedAssigneeCombo.isEmpty()) {
                Notification.show("Vui lòng nhập lý do và chọn người thực hiện mới")
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }

            try {
                taskService.requestChangeTask(
                        task.getId(),
                        reasonArea.getValue(),
                        requestedAssigneeCombo.getValue().getUserId()
                );
                Notification.show("Đã gửi yêu cầu đổi công việc")
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                refreshGridData();
                dialog.close();
            } catch (Exception ex) {
        ...
        dialog.add(new VerticalLayout(reasonArea, requestedAssigneeCombo));
        dialog.getFooter().add(new Button("Hủy", ce -> dialog.close()), submitBtn);
        dialog.open();
    }

    private void openViewReasonPopup(ShowTaskResponse task) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Chi tiết yêu cầu của thành viên");
        dialog.setWidth("500px");
        ...
        Span taskLabel = new Span("Công việc: " + task.getTaskName());
        taskLabel.getStyle().set("font-weight", "bold");

        TextArea reasonTextArea = new TextArea("Lý do thành viên gửi:");
        reasonTextArea.setValue(task.getRequestReason() != null ? task.getRequestReason() : "Không có lý do");
        reasonTextArea.setReadOnly(true);
        reasonTextArea.setWidthFull();
        reasonTextArea.setHeight("160px");

        dialogLayout.add(taskLabel, reasonTextArea);

        if (task.getStatus() == Task.TaskStatus.EXTENSION_REQUESTED) {
            String deadlineText = task.getRequestedDeadline() != null
                    ? task.getRequestedDeadline().format(dateFormatter)
                    : "Chưa chọn deadline mới";
            dialogLayout.add(new Span("Deadline mong muốn: " + deadlineText));
        }

        if (task.getStatus() == Task.TaskStatus.CHANGE_REQUESTED) {
            String requestedAssignee = task.getRequestedAssigneeFullName() != null
                    ? task.getRequestedAssigneeFullName()
                    : (task.getRequestedAssigneeUsername() != null ? task.getRequestedAssigneeUsername() : "Chưa chọn người mới");
            dialogLayout.add(new Span("Người thực hiện mới mong muốn: " + requestedAssignee));
        }
